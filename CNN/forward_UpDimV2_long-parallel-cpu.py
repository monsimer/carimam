...
def main(args):
    ...
    model.load_state_dict((torch.load(args.weight,map_location=device)['model']))
#     model.to('cuda')
    model.eval()
        ...
        if args.input_path.endswith('.npy'):
            ...
            with torch.no_grad():
                ...
                for i in trange(len(click_data)//batch_size, desc=f'file: {args.input_path}'):
                    ...
                    clicks = torch.from_numpy(((clicks - clicks.mean(-1, keepdims=True))/(clicks.std(-1, keepdims=True) + 1e-18))[:, np.newaxis]).float()
